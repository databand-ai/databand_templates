...
def monitor_S3_key(**context):
    ...
    bucketname = context["target_bucket"]
    key_name = path.join(*context["target_key"])
    log_metric("target file", target_path)

    # boto3_key_object = s3_hook.get_key(key=target_path)
    boto3_key_object = s3_session.resource('s3', endpoint_url=ENDPOINT_URL).Object(bucketname, key_name)
...
def monitor_S3_prefix(**context):
    ...
    bucket = s3_session.resource('s3', endpoint_url=ENDPOINT_URL).Bucket(target_bucket)
...
with dag as s3_bucket_template_dag:
    ...
    if TARGET_KEYS:
        ...
        for URI, bucket, key in zip(target_URIs, target_buckets, target_keys):
            basename = key[-1]
            ...
            AirflowTasks.append(
                PythonOperator(
                    ...
                    op_kwargs={"target_s3_path": URI, "path_basename": basename, "target_bucket": bucket, 'target_key':key},
                )
            )
